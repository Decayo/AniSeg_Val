...
if os.getenv('volna') is not None:
    ...
    #C.volna = r'J:\master_1_down\_thesis\code\main_workspace\AniSeg_Edge_Module_0414\0425val'
    C.volna = r'J:\master_1_down\_thesis\code\main_workspace\AniSeg_Val'
"""please config ROOT_dir and user when u first using"""
C.repo_name = 'AniSeg_Val'
C.abs_dir = osp.realpath(".")
C.this_dir = C.abs_dir.split(osp.sep)[-1]

C.root_dir = C.abs_dir[:C.abs_dir.index(C.repo_name) + len(C.repo_name)]
C.log_dir = r'J:\master_1_down\_thesis\code\main_workspace\AniSeg_Val\log'
...
C.dataset_path = osp.join(C.volna)
...
C.eval_model_path = r'J:\master_1_down\_thesis\code\main_workspace\AniSeg_Edge_Module_0414\log\0528_nc_re_a100\epoch-40.pth'
""" Path Config """
def add_path(path):
...
C.eval_source = osp.join(C.dataset_path, "val_all.txt")
C.is_test = False
C.fix_bias = True
C.bn_eps = 1e-5
C.bn_momentum = 0.1
